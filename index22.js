...
AFRAME.registerComponent('artgalleryframe', {
  ...
  init() {
    ...
    const {title, note1, note2, note3} = JSON.parse(this.data.metadata)
    ...
    infoDisplay.setAttribute('info-display', {title, note1, note2})
    ...
    //   })
    pageContent = `<h1>${this.data.title}</h1>${this.data.note1}` 
...
AFRAME.registerComponent('info-display', {
  ...
  init() {
    ...
    const text = `${displayTitle}\n${this.data.note1}, ${this.data.note2}`
    const textData = {
      align: 'left',
      width: 0.7,
      wrapCount: 22,
